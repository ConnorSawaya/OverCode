...
import { createOpencode } from "@overcode-ai/sdk"
...
`)
  ...
  const overcode = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await overcode.client.session.create()
    const result = await overcode.client.session
      .prompt({
