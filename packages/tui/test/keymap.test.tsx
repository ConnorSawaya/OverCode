...
import { getOvercodeModeStack, OVERCODE_BASE_MODE, OvercodeKeymapProvider, registerOvercodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerOvercodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when overcode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getOvercodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getOvercodeModeStack(keymap).push("autocomplete")
