import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@overcode-ai/plugin/tui"
...
import type { useOvercodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useOvercodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getOvercodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getOvercodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
