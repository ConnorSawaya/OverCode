...
import { createOpencode, type ToolPart } from "@overcode-ai/sdk"
...
console.log("🚀 Starting overcode server...")
const overcode = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await overcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new overcode session...")
    const { client, server } = overcode
    ...
    console.log("✅ Created overcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to overcode:", message.text)
