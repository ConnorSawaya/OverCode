...
import type { SessionNotFoundError } from "@overcode-ai/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em overcode.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
