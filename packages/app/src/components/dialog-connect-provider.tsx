import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@overcode-ai/client/promise"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "overcode-go") return language.t("dialog.provider.overcodeGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "overcode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.overcode.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["overcode", "overcode-go", "anthropic", "openai", "google", "openrouter", "vercel"]
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "overcode"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.overcodeZen.line1")}</div>
              <div>{language.t("provider.connect.overcodeZen.line2")}</div>
              <div>
                {language.t("provider.connect.overcodeZen.visit.prefix")}
                <ExternalLink
                  href="https://overcode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.overcodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.overcodeZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "overcode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.overcodeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.overcodeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.overcodeZen.visit.prefix")}
                <ExternalLink href="https://overcode.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.overcodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.overcodeZen.visit.suffix")}
              </div>
