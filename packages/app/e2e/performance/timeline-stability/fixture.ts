...
import { SessionV1 } from "@overcode-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Overcode/TimelineStability"
...
export const model = { providerID: "overcode", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("overcode.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
