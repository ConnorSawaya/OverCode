...
import { LocationServiceMap } from "@overcode-ai/core/location-service-map"
import { SessionExecutionLocal } from "@overcode-ai/core/session/execution/local"
...
export function createRoutes(password?: string) {
  return makeRoutes(
    password
      ? ServerAuth.Config.configLayer({ username: "overcode", password: Option.some(password) })
      : ServerAuth.Config.layer,
  )
}

export function createEmbeddedRoutes() {
  return makeRoutes(ServerAuth.Config.configLayer({ username: "overcode", password: Option.none() }))
}
