...
import { PromptInput } from "@overcode-ai/schema/prompt-input"
import { Session } from "@overcode-ai/schema/session"
import { Project } from "@overcode-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@overcode-ai/schema/schema"
...
import { SessionEvent } from "@overcode-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Overcode process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Overcode process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
