...
import { EffectDrizzleSqlite } from "@overcode-ai/effect-drizzle-sqlite"
import { Effect, Layer } from "effect"
import { eq, inArray, sql } from "drizzle-orm"
import { DatabaseMigration } from "@overcode-ai/core/database/migration"
import { migrations } from "@overcode-ai/core/database/migration.gen"
import workspaceNameMigration from "@overcode-ai/core/database/migration/20260410174513_workspace-name"
import sessionUsageMigration from "@overcode-ai/core/database/migration/20260510033149_session_usage"
import normalizeStoragePathsMigration from "@overcode-ai/core/database/migration/20260601010001_normalize_storage_paths"
import sessionMessageProjectionOrderMigration from "@overcode-ai/core/database/migration/20260603040000_session_message_projection_order"
import eventSourcedSessionInputMigration from "@overcode-ai/core/database/migration/20260604172448_event_sourced_session_input"
import contextEpochAgentMigration from "@overcode-ai/core/database/migration/20260605042240_add_context_epoch_agent"
import simplifyIntegrationCredentialsMigration from "@overcode-ai/core/database/migration/20260611192811_lush_chimera"
import simplifySessionInputMigration from "@overcode-ai/core/database/migration/20260622202450_simplify_session_input"
...
import { SessionSchema } from "@overcode-ai/core/session/schema"
import { SessionTable } from "@overcode-ai/core/session/sql"
import sessionMetadataMigration from "@overcode-ai/core/database/migration/20260511173437_session-metadata"
  ...
  test("applies tracked migrations to an empty database", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(yield* db.get(sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'sync_state'`)).toEqual({
          name: "sync_state",
        })
        expect(yield* db.get(sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'sync_change'`)).toEqual({
          name: "sync_change",
        })
        expect(yield* db.get(sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'sync_profile'`)).toEqual({
          name: "sync_profile",
        })
        expect(
          yield* db.get(sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'sync_project_mapping'`),
        ).toEqual({ name: "sync_project_mapping" })
        expect(
          yield* db.get(
