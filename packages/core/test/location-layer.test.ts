...
import { define } from "@overcode-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@overcode-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "overcode.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "overcode.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
