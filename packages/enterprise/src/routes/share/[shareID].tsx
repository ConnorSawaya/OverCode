import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@overcode-ai/sdk/v2"
import { SessionTurn } from "@overcode-ai/session-ui/session-turn"
import { SessionReview } from "@overcode-ai/session-ui/session-review"
import { DataProvider } from "@overcode-ai/session-ui/context"
import { FileComponentProvider } from "@overcode-ai/ui/context/file"
import { WorkerPoolProvider } from "@overcode-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@overcode-ai/ui/logo"
import { IconButton } from "@overcode-ai/ui/icon-button"
import { ProviderIcon } from "@overcode-ai/ui/provider-icon"
import { iife } from "@overcode-ai/core/util/iife"
...
import { MessageNav } from "@overcode-ai/session-ui/message-nav"
import { FileSSR } from "@overcode-ai/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@overcode-ai/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/overcode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Overcode</Title>
              </Show>
              <Meta name="description" content="overcode - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              ...
                              <IconButton
                                as={"a"}
                                href="https://github.com/ConnorSawaya/overcode/discussions"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
