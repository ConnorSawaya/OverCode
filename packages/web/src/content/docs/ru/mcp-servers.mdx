...
Вы можете определить серверы MCP в своем [overcode Config](https://github.com/ConnorSawaya/overcode/tree/plain-cli/packages/web/src/content/docs/config/) в разделе `mcp`. Добавьте каждому MCP уникальное имя. Вы можете обратиться к этому MCP по имени при запросе LLM.
