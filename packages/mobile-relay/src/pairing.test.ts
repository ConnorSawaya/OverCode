...
describe("mobile pairing", () => {
  ...
  test("only permits loopback HTTP relay URLs", () => {
    ...
    expect(parsePairingUri("overcode://mobile?relay=http%3A%2F%2Frelay.example.test&code=48291735")).toBeUndefined()
  })

  test("round trips short-lived eight-digit codes", () => {
    const uri = createPairingUri({ relay: "https://relay.example.test", code: "48291735" })
    expect(uri).toContain("code=48291735")
    expect(parsePairingUri(uri)).toEqual({ relay: "https://relay.example.test", code: "48291735" })
    expect(isPairingCode("4829 1735")).toBe(true)
    expect(formatPairingCode("48291735")).toBe("4829 1735")
    expect(isPairingCode("4829173")).toBe(false)
    expect(isPairingCode("482917")).toBe(false)
  })
})
