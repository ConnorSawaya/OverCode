...
import type { LLMClientShape } from "@overcode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("overcode"))
    return { type: "unsupported", reason: "provider is not openai, overcode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @overcode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains overcode-owned. The native runtime only adapts
      // the @overcode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
