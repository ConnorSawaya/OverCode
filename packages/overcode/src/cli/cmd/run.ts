import type { PermissionV1 } from "@overcode-ai/core/v1/permission"
import { FSUtil } from "@overcode-ai/core/fs-util"
// CLI entry point for `overcode run` and `overcode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`overcode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`overcode --mini --attach`): connects to a running
//      overcode server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@overcode-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run overcode with a message",
