...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@overcode-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@overcode-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@overcode-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@overcode-ai/tui/context/theme"
import { Global } from "@overcode-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@overcode-ai/core/util/flock"
import { Flag } from "@overcode-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@overcode-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@overcode-ai/core/v1/config/plugin"
import { createCommandShim } from "@overcode-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@overcode-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".overcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".overcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".overcode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.OVERCODE_PURE ? [] : pluginOrigins
    if (Flag.OVERCODE_PURE && pluginOrigins.length) {
    }
