...
import { IdeEvent } from "@overcode-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["OVERCODE_CALLER"] === "vscode" || process.env["OVERCODE_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.overcode"], {
    nothrow: true,
  })
