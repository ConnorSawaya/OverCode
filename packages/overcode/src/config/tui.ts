...
import { isRecord } from "@overcode-ai/tui/util/record"
...
import { TuiKeybind } from "@overcode-ai/tui/config/keybind"
...
import { TuiConfig } from "@overcode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@overcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old overcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.overcode`
  // folders between cwd and home, and OVERCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.OVERCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit OVERCODE_TUI_CONFIG override, if set.
  if (Flag.OVERCODE_TUI_CONFIG) {
    const configFile = Flag.OVERCODE_TUI_CONFIG
  ...
  // 4. `.overcode` directories (and OVERCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".overcode") || dir === Flag.OVERCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".overcode") && dir !== Flag.OVERCODE_CONFIG_DIR) continue
