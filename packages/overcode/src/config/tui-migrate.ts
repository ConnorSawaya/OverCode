...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from overcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const overcode = await overcodeFiles(input)
  for (const file of overcode) {
...
async function overcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "overcode"),
    ...(await Filesystem.findUp(["overcode.json", "overcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "overcode"))
  }
  if (Flag.OVERCODE_CONFIG) files.push(Flag.OVERCODE_CONFIG)
