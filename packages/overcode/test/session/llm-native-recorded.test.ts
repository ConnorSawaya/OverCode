...
import { HttpRecorder } from "@overcode-ai/http-recorder"
import { HttpRecorderInternal } from "@overcode-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@overcode-ai/llm"
import { RequestExecutor } from "@overcode-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.overcode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.OVERCODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["overcode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OVERCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("OVERCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["overcode", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "overcode-proxy",
    name: "Overcode proxy",
    providerID: ProviderV2.ID.overcode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["overcode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.OVERCODE_RECORD_CONSOLE_TOKEN && process.env.OVERCODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.overcode,
        name: "Overcode Zen",
        env: ["OVERCODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.OVERCODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.OVERCODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.OVERCODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["overcode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OVERCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("OVERCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("OVERCODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set OVERCODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the overcode LLM stack remain real.
  const metadata = {
