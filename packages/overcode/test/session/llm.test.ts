...
import { LLMClient, RequestExecutor } from "@overcode-ai/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const overcodeFixture = { providerID: "overcode-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for overcode providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(overcodeFixture.providerID),
          ModelV2.ID.make(overcodeFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(overcodeFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [overcodeFixture.providerID],
          provider: {
            [overcodeFixture.providerID]: {
              name: "Overcode Test",
