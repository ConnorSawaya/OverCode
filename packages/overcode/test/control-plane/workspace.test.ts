...
import { EventSequenceTable } from "@overcode-ai/core/event/sql"
...
const originalEnv = {
  OVERCODE_AUTH_CONTENT: process.env.OVERCODE_AUTH_CONTENT,
  OVERCODE_EXPERIMENTAL_WORKSPACES: process.env.OVERCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.OVERCODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@overcode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.OVERCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=overcode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.OVERCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.OVERCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.OVERCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=overcode-test")
