...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "overcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "overcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "overcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "overcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["overcode.json", "overcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "overcode.json"),
        path.join(project, "overcode.jsonc"),
      ])
    })
  })
  ...
  describe("writeStream()", () => {
    ...
    test("writes binary data from Web ReadableStream", async () => {
      ...
      const read = await fs.readFile(filepath)
      expect(read).toEqual(Buffer.from(binaryData))
    })
