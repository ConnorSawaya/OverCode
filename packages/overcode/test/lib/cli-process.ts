// Subprocess test harness for the overcode CLI. Spawns the real binary against
...
//
// Configuration flows through overcode's built-in test affordances:
//   - OVERCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - OVERCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - OVERCODE_DISABLE_PROJECT_CONFIG : skip walking up for overcode.json
//   - OVERCODE_PURE                : skip external plugin discovery + install
//   - OVERCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `overcode.run` is fully wired. The shape supports adding more
// builders (`overcode.serve(opts)`, `overcode.acp(opts)`, `overcode.auth(...)`)
...
import { AppProcess } from "@overcode-ai/core/process"
...
const overcodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(overcodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    OVERCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    OVERCODE_CONFIG_CONTENT: configJson,
    OVERCODE_DISABLE_PROJECT_CONFIG: "1",
    OVERCODE_PURE: "1",
    OVERCODE_DISABLE_AUTOUPDATE: "1",
    OVERCODE_DISABLE_AUTOCOMPACT: "1",
    OVERCODE_DISABLE_MODELS_FETCH: "1",
    OVERCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `overcode run`. New flags should
...
// `overcode serve` is a long-lived process — it never exits on its own.
...
// `overcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `overcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `overcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("overcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          OVERCODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("overcode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("overcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "overcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `overcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("overcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const overcode: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, overcode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `overcode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "overcode") {
...
// every call site. Use this for any test that needs the overcode CLI fixture.
//
...
// resources (e.g. `overcode.serve`) without an extra `Effect.scoped` wrapper —
