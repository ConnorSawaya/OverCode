 ...
 *
 * The script intentionally isolates `OVERCODE_DB` before importing modules that touch
...
import { controlledPtyInput, controlledPtyShell, http, route } from "./dsl"
import {
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "overcode.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "overcode.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-overcode-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-overcode-directory")
  }),
  ...
  http.protected
    ...
    .json(
      ...
      (body, ctx) => {
        ...
        check(body.command === controlledPtyShell().command, "PTY create should use controlled shell command")
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "overcode", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "x-overcode-ticket": "1" },
    }))
    .json(404, object, "status"),
  http.protected
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "content-type": "application/json" },
      body: { model: { providerID: "overcode", id: "big-pickle" } },
    }))
    .status(204, undefined, "none"),
  http.protected
